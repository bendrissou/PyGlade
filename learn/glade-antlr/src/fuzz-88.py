...
class CheckFuzzer(Fuzzer):
    ...
    def gen_key(self, key, depth, max_depth):
        ...
        if key != self.key and self.key in chosen_rule and self.check == 2: 
            print("\nRules: " + str(rules))
            print("Chosen rule: " + str(chosen_rule))
            print("Current key: " + key)
  
        old_chosen_rule = chosen_rule
        ...
        if key != self.key and self.key in old_chosen_rule and self.check == 2 and current_expansion in self.past_expansions: 
            print("Edited Chosen rule: " + str(chosen_rule))
            self.count += 1
            if self.count > 1:
                print("+++++++")
                chosen_rule = ['' if token == self.ini_token else token for token in chosen_rule]
            #chosen_rule = ['' if token == self.ini_token else token for token in chosen_rule]
            #chosen_rule = old_chosen_rule
                current_expansion = key + ''.join(chosen_rule)
...
def main(fn):
    ...
    with open("../../synthesized/" + fn + "_.json") as f:
