...
def char_gen_phase(regex):
    ...
    while True:
        ...
        if regex == NON_GENERALIZABLE:
            ...
            for expr in exprs:
                checks += 1
                v = check.check(expr, program, regex)
...
def to_key(prefix, suffix=''):
    # Add random string of length 5 to the key name, to avoid potential naming conflicts.
...
def extract_alt(regex, prefix):
    ...
    for i in range(5): randompre.append(choice(ascii_lowercase))
    prefix = prefix + randompre
    g[to_key(prefix, '_alt')] = [[k1], [k2]]
    return g, to_key(prefix, '_alt')
...
def extract_one(regex, prefix):
    if len(regex.o) == 1: # one is not a non terminal
        ...
        for t in regex.o:
            alts.append([t])
        randompre = ['_']
        print(choice(ascii_lowercase) for i in range(5))
        for i in range(5): randompre.append(choice(ascii_lowercase))
        prefix = prefix + randompre
        g[to_key(prefix, '_chr')] = alts
        return g, to_key(prefix, '_chr')
